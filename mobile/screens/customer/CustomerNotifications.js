...
const CustomerNotifications = ({ navigation }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const serverEnabled = preferenceResponse.data?.data?.enabled !== false;
      setNotifications(listResponse.data?.data || []);
      setEnabled(serverEnabled);
      await AsyncStorage.setItem('notificationsEnabled', String(serverEnabled));
    } catch (error) {
      console.warn('Customer notifications unavailable:', error?.response?.status || error?.message);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const toggleNotifications = async (value) => {
    ...
    try {
      ...
      setEnabled(!value);
      console.warn('Customer notification preference sync failed:', error?.response?.status || error?.message);
    }
  };
  ...
  const markRead = async (notification) => {
    ...
    try {
      ...
      console.warn('Notification read state sync failed:', error?.response?.status || error?.message);
      load();
    }
  };
...
const styles = StyleSheet.create({
  ...
  preference: { margin: 16, padding: 16, borderRadius: 16, backgroundColor: '#FFFFFF', flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center' },
