...
const VendorDashboard = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const [ordersResult, statsRes] = await Promise.allSettled([
        orderService.getVendorOrders(user?.id),
      ...
      if (ordersResult.status === 'fulfilled' && ordersResult.value.success) {
        const data = ordersResult.value.orders || [];
      ...
      if (statsRes.status === 'fulfilled') {
        ...
        if (s) {
          setStats({
            ...
            preparing: s.preparingOrders || 0,
            completed: s.totalOrders - s.pendingOrders - s.preparingOrders,
