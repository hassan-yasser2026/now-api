...
app.post('/api/assistant/chat', authMiddleware, async (req, res) => {
  ...
  if (!apiKey) {
    return errorResponse(res, 'خدمة المساعد الذكي غير مفعلة حالياً', 503);
  }
  ...
  if (!validMessages.length || validMessages[validMessages.length - 1].role !== 'user') {
    return errorResponse(res, 'أرسل رسالة صحيحة للمساعد', 400);
  }
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'أنت مساعد NOW الذكي. أجب بالعربية المصرية باختصار ووضوح. ساعد المستخدم في المتاجر والمنتجات والطلبات والتوصيل والدفع. لا تخترع حالة طلب أو سعرًا أو سياسة غير موجودة. إذا احتاج الأمر بيانات حساب أو طلب، اطلب من المستخدم رقم الطلب أو وجهه للدعم.',
          },
          ...validMessages,
        ],
      }),
    ...
    if (!response.ok) {
      console.error('OPENAI ASSISTANT ERROR:', response.status, payload?.error?.message);
      return errorResponse(res, 'تعذر الحصول على رد من المساعد حالياً', 502);
    }

    const reply = payload?.choices?.[0]?.message?.content?.trim();
    if (!reply) {
      return errorResponse(res, 'لم يصل رد صالح من المساعد', 502);
    }

    return successResponse(res, { reply });
  } catch (error) {
    console.error('ASSISTANT ERROR:', error);
    return errorResponse(res, 'تعذر الاتصال بخدمة المساعد حالياً', 502);
  }
});
